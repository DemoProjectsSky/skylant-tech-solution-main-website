...
// Stagger container for the stats row so each stat animates in one after another
...
export default function PageBanner({
  ...
  <section className="relative pt-4 pb-16 lg:pt-6 lg:pb-20 overflow-hidden">
      {/* Background — lavender / off-white theme */}
      ...
      />
      {/* faint fade so text stays readable over the wash */}
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <motion.nav
              ...
              className="flex items-center gap-2 text-sm text-[#8B85A8] mb-6"
            >
              {breadcrumb.map((b, i) => (
                <span key={i} className="flex items-center gap-2">
                  {b.path ? (
                    <Link to={b.path} className="hover:text-[#6D5BD0] transition-colors">{b.name}</Link>
                  ) : (
                    <span className="text-[#4B3F91]">{b.name}</span>
                  )}
                  {i < breadcrumb.length - 1 && <ChevronRight className="w-3.5 h-3.5 text-[#C9BAFF]" />}
                </span>
              ))}
            </motion.nav>
              ...
              >
                {primaryCta && (
                  <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                    <Link to={primaryCta.path} className="group inline-flex items-center bg-[#1D4ED8] hover:bg-[#1E40AF] text-white px-6 py-3 rounded-lg transition-all duration-300">
                      {primaryCta.label}
                      <motion.span
                ...
                {secondaryCta && (
                  <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                    <Link
                      ...
                      className="inline-flex items-center justify-center px-6 py-3 rounded-xl bg-transparent border-2 border-[#1D4ED8] text-black transition-all duration-300 hover:shadow-[0_0_20px_rgba(29,78,216,0.45)] hover:scale-105"
                    >
                      {secondaryCta.label}
                    </Link>
                  </motion.div>
              ...
              >
                {stats.map((s, i) => (
                  <motion.div key={i} variants={statItem}>
                    <div className="text-2xl lg:text-3xl font-bold text-[#4B3F91]">{s.value}</div>
                    <div className="text-sm text-[#8B85A8] mt-1">{s.label}</div>
                  </motion.div>
                ))}
              </motion.div>
            )}
          ...
          {/* Right - Animated image / illustration with floating cards */}
          <motion.div
          ...
          >
            {illustration ? (
              // Custom animation slot — renders directly on the page background,
              // no card box, no glow ring, no shine sweep.
              <motion.div
              ...
              <div className="relative aspect-[4/4.2] rounded-3xl overflow-hidden">
                ...
                {/* Rotating glow ring around the border */}
                <motion.div
                  className="absolute -inset-[2px] rounded-3xl"
                  style={{
                    background: `conic-gradient(from 0deg, ${accentColor}00, ${accentColor}CC, ${accentColor}00 30%)`,
                  }}
                  animate={{ rotate: 360 }}
                ...
                >
                  <motion.img
                    ...
                    transition={{
                      ...
                      clipPath: { duration: 1.1, ease: [0.65, 0, 0.35, 1] },
                      scale: { duration: 14, repeat: Infinity, ease: 'easeInOut', delay: 1.1 },
                      x: { duration: 14, repeat: Infinity, ease: 'easeInOut', delay: 1.1 },
                      y: { duration: 14, repeat: Infinity, ease: 'easeInOut', delay: 1.1 },
                    }}
                  />
                  ...
                  <motion.div
                    ...
                    initial={{ x: '-130%' }}
                    animate={{ x: '130%' }}
                    transition={{ duration: 2.2, repeat: Infinity, repeatDelay: 3.5, ease: 'easeInOut', delay: 1.5 }}
                  />
            ...
            {floatingCards.slice(0, 4).map((card, i) => {
              ...
              return (
                <motion.div
                  ...
                  whileHover={{ scale: 1.1, y: -6, rotate: i % 2 === 0 ? -2 : 2 }}
                  transition={{ duration: 0.5, delay: 0.5 + i * 0.15 }}
                  className={`absolute ${positions[i]} glass-card-strong rounded-2xl p-3.5 shadow-xl shadow-black/30 cursor-default ${i % 2 === 0 ? 'animate-float' : 'animate-float-delay'}`}
                >
                  <div className="flex items-center gap-2.5">
                    <motion.div
                      ...
                      transition={{ duration: 4, repeat: Infinity, repeatDelay: 2, delay: i * 0.3 }}
                    >
                    ...
                    <div>
                      <div className="text-[#2C2A4A] text-xs font-semibold whitespace-nowrap">{card.title}</div>
                      {card.subtitle && <div className="text-[#8B85A8] text-[10px]">{card.subtitle}</div>}
                    </div>
                  </div>
                </motion.div>
              );
